...
--On the Payloads tab, add the list of usernames to the first payload set. For the second set, select the Null payloads type and choose the option to 
--generate 5 payloads. This will effectively cause each username to be repeated 5 times. Start the attack.
...
--Create a new Burp Intruder attack on the POST /login request, but this time select the Sniper attack type.
...
--In the results, look at the grep extract column. Notice that there are a couple of different error messages, but one of the 
--responses did not contain any error message. Make a note of this password.
--Wait for a minute to allow the account lock to reset. Log in using the username and password that you identified and access the user account
